    ...
    <div className="w-full mb-12 flex flex-col gap-6">
      ...
      <div className="flex justify-between items-center py-6 border-y border-plata-niebla/10 bg-hueso-seda z-30">
        <div className="flex gap-12 items-center">
          <button 
            ...
            className="flex items-center gap-3 text-xs uppercase tracking-widest hover:text-oro-antiguo transition-colors"
          >
            <Filter size={14} /> {isSidebarOpen ? "Cerrar Filtros" : "Filtrar por..."}
          </button>
          
          <div className="hidden md:flex gap-12 text-xs uppercase tracking-widest text-plata-niebla">
        ...
        <div className="flex items-center gap-8">
          <div className="hidden md:flex items-center gap-4">
            <span className="text-xs uppercase tracking-widest text-plata-niebla">Ordenar:</span>
            <select className="bg-transparent text-xs uppercase tracking-widest outline-none border-none cursor-pointer hover:text-oro-antiguo transition-colors">
              <option>Destacados</option>
              <option>Precio: Mayor a Menor</option>
              <option>Precio: Menor a Mayor</option>
            </select>
          </div>
          <span className="text-xs uppercase tracking-widest font-medium">84 Piezas</span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-12 relative">
            ...
            >
              {filterSections.map((section) => (
                <div key={section.title} className="flex flex-col gap-6">
                  <h4 className="text-xs uppercase tracking-[0.2em] text-oro-antiguo font-medium border-l border-oro-antiguo pl-3">{section.title}</h4>
                  <div className="flex flex-col gap-3">
                    {section.options.map((opt) => (
                      <label key={opt} className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span className="text-[11px] uppercase tracking-widest text-verde-ebano/60 group-hover:text-verde-ebano transition-colors">{opt}</span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-8 p-6 bg-authority flex flex-col gap-4">
                <div className="flex items-center gap-2 text-oro-antiguo">
                  <Sparkles size={14} />
                  <span className="text-[11px] uppercase tracking-widest">Atelier IA</span>
                </div>
                <p className="text-[11px] text-hueso-seda/50 leading-relaxed uppercase tracking-tighter">
                  Describa su visión para una recomendación única.
                </p>
                <textarea 
                  rows={3}
                  className="bg-hueso-seda/5 border border-hueso-seda/10 p-3 text-xs text-hueso-seda focus:border-oro-antiguo outline-none transition-all resize-none"
                  placeholder="Ej: Collares minimalistas para gala..."
                />
                <button className="text-[11px] uppercase tracking-widest text-oro-antiguo flex items-center justify-between group">
        ...
        <div className="flex-1 min-h-[1000px] bg-plata-niebla/5 border border-dashed border-plata-niebla/20 flex items-center justify-center">
          <span className="text-xs uppercase tracking-widest opacity-20">Contenedor de Galería de Productos</span>
        </div>
      </div>
    </div>
  );
